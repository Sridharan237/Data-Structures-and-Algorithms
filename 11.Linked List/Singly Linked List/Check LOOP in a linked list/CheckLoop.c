//C program to check whether a loop is present in a linked list or not
//linked list - linear (or) loop
...
// function to merge 2 sorted linked lists to a single sorted linked list
...
// function to detect whether loop is present in a linked list using two pointers : p, q
int isLoop(struct Node *f)  // f - first node of a linked list
{
    struct Node *p, *q;
    p = q = first;
    ...
    do
    {
        p = p->next;    // move 1 step

        q = q->next;    // move 2 steps
        q = (q!=NULL)?(q->next):q;
    } while (p && q && p != q);

    if(p == q)  return 1;   // loop is present
    else    return 0;       // loop is not present (ie, linear linked list)
...
int main()
{
    ...
    // making loop in linked list first 
    t1 = first->next->next;
