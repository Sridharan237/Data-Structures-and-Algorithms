...
// function for deleting a node in a singly circular linked list
int Delete(struct Node *p, int index)
{
    ...
    if(index == 1)
    {
        ...
        if(p == Head)   // Only 1 node exists (ie, Head node Only)
        {
            free(Head);
            Head = NULL;
        }
        else    // some more nodes exists along with Head node
        {
            p->next = Head->next;
            free(Head);
            Head = p->next;
